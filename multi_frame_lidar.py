...
def accumulate_frames(frames, seq_dir, poses, T_cam0_from_velo):
    """Return world_points 4xN and per-point frame indices."""
    all_world = []
    all_frame_ids = []

    for k in frames:
        velo_path = os.path.join(seq_dir, "velodyne", f"{k:06d}.bin")
        pts = load_velodyne_bin(velo_path)
        X_velo = make_homog(pts)

        T_w_from_cam0_k = poses[k]
        T_w_from_velo_k = T_w_from_cam0_k @ np.linalg.inv(T_cam0_from_velo)

        X_world = T_w_from_velo_k @ X_velo
    ...
    return all_world, all_frame_ids
    ...
    for i in range(len(ui)):
        ...
        for yy in range(y0, y1 + 1):
            ...
            for xx in range(x0, x1 + 1):
                ...
                g = np.exp(-r2 / two_sigma2) * float(w)
                if g < 1e-4:
                    continue
...
def main(args):
    ...
    frames = [t - 1, t, t + 1]
    # frames = [t - 3, t - 2, t - 1, t, t + 1, t + 2, t + 3]
    ...
    # ----------------------
    # 1. Multi-frame world fusion
    # ----------------------
    world_points, frame_ids = accumulate_frames(frames, seq_dir, poses, T_cam0_from_velo)
    weights = temporal_weights(frame_ids, t)

    # ----------------------
    # 2. World -> target cam2
    # ----------------------
    T_w_from_cam0_t = poses[t]
    X_world = world_points  # already 4xN
    X_cam2_t = T_cam2_from_cam0 @ np.linalg.inv(T_w_from_cam0_t) @ X_world
    ...
    # 7. Save outputs
    # ----------------------
    outdir = f"Multi_frame_fuse_{args.splat_type}"
